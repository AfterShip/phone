...
function get_iso3166_by_phone(phone) {
	...
	for (var i in iso3166_data) {
		...
		for (var j in iso3166_data.phone_number_lengths) {
			if (phone.match(regex) && phone.length === iso3166_data[i].country_code.length + iso3166_data[i].phone_number_lengths[j]) {
				...
				for (var k in iso3166_data[i].mobile_begin_with) {
					regex = new RegExp('^' + iso3166_data[i].country_code + iso3166_data[i].mobile_begin_with[k]);
...
function validate_phone_country(phone, iso3166) {
	phone = phone.replace(RegExp('^' + iso3166.country_code), '');
	for (var i in iso3166.phone_number_lengths) {
		if (phone.length === iso3166.phone_number_lengths[i]) {
			for (var j in iso3166.mobile_begin_with) {
				if (phone.match(RegExp('^' + iso3166.mobile_begin_with[j]))) {
					return true;
				}
			}
		}
...
module.exports = function(phone, country) {
	...
	if (!country) {
		if (plus_sign) {
			...
			if (phone.length === iso3166.phone_number_lengths[0]) {
				phone = '1' + phone;
			}
			console.log('after');
			console.log(phone);
	...
	if (validate_phone_country(phone, iso3166)) {
