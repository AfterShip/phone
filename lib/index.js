...
import get_iso3166_by_phone from './getISO3166ByPhone';
import validate_phone_iso3166 from './validatePhoneISO3166';
export {default as iso3166_data} from './iso3166Data';
...
export default function(phone, country) {
	const result = [];
	let formatPhone =
		phone === null || typeof phone !== 'string' ? '' : phone.trim();
	const formatCountry =
	...
	if (formatCountry) {
		...
		if (
			!plus_sign &&
			iso3166.phone_number_lengths.includes(formatPhone.length)
		) {
