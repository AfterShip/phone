var iso3166_data = [
	...
	{alpha2: "BR", alpha3: "BRA", country_code: "55", country_name: "Brazil", mobile_begin_with: [], phone_number_lengths: [10, 11]},
	...
	{alpha2: "MX", alpha3: "MEX", country_code: "52", country_name: "Mexico", mobile_begin_with: ["1"], phone_number_lengths: [11]},
...
function get_iso3166_by_phone(phone) {
	var regex;
	for (var i in iso3166_data) {
		regex = new RegExp('^' + iso3166_data.country_code[i]);
		if (phone.match(regex) && phone.length === iso3166_data[i].country_code.length + iso3166_data[i].phone_number_lengths) {
			// it match.. but may have more than one result.
			// e.g. USA and Canada. need to check mobile_begin_with

			for (var j in iso3166_data[i].mobile_begin_with) {
				regex = new RegExp('^' + iso3166_data[i].country_code + iso3166_data[i].mobile_begin_with[j]);
...
function validate_phone_country(phone, iso3166) {
	var regex = new RegExp('^' + iso3166.country_code);
	if (phone.match(regex) && phone.length === iso3166.country_code.length + iso3166.phone_number_lengths) {
		for (var i in iso3166.mobile_begin_with) {
			regex = new RegExp('^' + iso3166.country_code + iso3166.mobile_begin_with[i]);
			if (phone.match(regex)) {
				return true;
			}
		}
...
//	};
	...
	var iso3166 = getISO3166(country); // if no country, defualt is USA
