 ...
 * @property {String[]} mobile_begin_with
 * @property {Number[]} phone_number_lengths
 */
 ...
 * @returns {String[]} ['+380990316789', 'UKR']
 */
 ...
 * @returns {PhoneData|null}
 */
module.exports.getDataForCountry = function getDataForCountry(country) {
	var result = null;
	if (!country) {
		return result;
	}
	...
	for(var i in iso3166) {
		if (iso3166.hasOwnProperty(i)) {
			var isArray = iso3166[i] instanceof Array || (iso3166[i] && iso3166[i].constructor === Array)
			result[i] = isArray ? iso3166[i].concat([]) : iso3166[i];
		}
	}
