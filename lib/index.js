...
function build_result(phone, iso3166) {
  ...
  if (area_code_support_list.indexOf(iso3166.alpha3) > -1) {
    ...
    phone = phone.replace(new RegExp('^' + iso3166.country_code), '');
    var area_code = iso3166.mobile_begin_with.find(function (mobile_begin_with) {
      return phone.match(new RegExp('^' + mobile_begin_with));
    });

    var slice_index = area_code.length;
    var sliced_phone = phone.slice(slice_index);
  ...
  var sliced_phone = phone.slice(iso3166.country_code.length);
...
module.exports = function (phone, country) {
  ...
  if (validate_phone_iso3166(phone, iso3166)) {
    return build_result(phone, iso3166);
  }
